...
fn build_servo_command(
    ...
    bombolab_core::communication::ServoCommand::new(joints_arr, gripper).map_err(|e| e.to_string())
}
...
async fn authenticate(
    ...
    if !ok {
        let _ = write
            .send(Message::Text(
                r#"{"type":"error","msg":"autenticación requerida"}"#.into(),
            ))
            .await;
    }
    if ok { Ok(()) } else { Err(()) }
...
async fn handle_client(
    ...
    // `ErrorResponse` carries the (possibly large) handshake error body.
    #[allow(clippy::result_large_err)]
    let ws =
        match tokio_tungstenite::accept_hdr_async(raw, |req: &Request, mut response: Response| {
    ...
    if let Some(expected) = auth_token
        && authenticate(&mut write, &mut read, &expected)
            .await
            .is_err()
    {
        return;
                        ...
                        Ok(_) => {
                            let _ = write
                                .send(tokio_tungstenite::tungstenite::Message::Text(
                                    serde_json::to_string(&OkMessage {
...
#[cfg(test)]
#[path = "ws-bridge/ws_bridge_tests.rs"]
mod ws_bridge_tests;
